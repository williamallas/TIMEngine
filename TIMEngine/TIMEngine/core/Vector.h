...
public:
    ...
    {
         T res=0;
         for(size_t i=0;i<std::min(N,A);i++) res+=(_val[i]*v[i]);
         for(size_t i=std::max(N,A)-1;i<std::max(N,A);i++)res+=v[i];
         return res;
    }
    ...
    template<size_t A> Vector<T,N-A> down() { static_assert(N-A>0, "Invalid vector size."); return to<N-A>(); }
    template<size_t A> Vector<T,N-A> extend() { return to<N+A>(); }
