...
Option<uint> MetaShader::compile(std::initializer_list<std::string> flags)
{
    return compile(boost::container::set<std::string>(flags.begin(), flags.end()));
}

Option<uint> MetaShader::compile(const boost::container::set<std::string>& flags)
{
    auto it = _shader.find(flags);
    if(it != _shader.end())
        return Option<uint>(it->second);
    ...
    uint id = glCreateShader(GLShaderType(_shaderType));
    ...
    return Option<uint>(id);
}

void MetaShader::logError(uint shaderId)
{
    int logSize;
